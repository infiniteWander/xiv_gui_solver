        ...
        for action in next_action_picker_1(&craft) {
            ...
            if remaining_prog <= 2.0 {
                if remaining_prog <= 0.0 {
                    ...
                    // TODO: Test replacing this value with 1.8 that should be the correct value but hasnt be tested
                    // } else if 1.2 < remaining_prog && remaining_prog <= 1.4 {
                    //     craft.cp -= 7; // Will finish with carefulSynthesis (7cp)
                } else if 1.4 < remaining_prog && remaining_prog <= 2.0 {
